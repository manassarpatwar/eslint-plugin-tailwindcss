...
/**
 * Find out if node is a skip-able property.
 * If it is an identifier it checks the name, if it is a literal it checks the value.
 *
 * @param {ASTNode} node The AST node being checked
 * @param {String} propertyRegex to test the attribute that is being checked against
 * @returns {Boolean}
 */
function isSkipProperty(node, propertyRegex) {
  let name = '';
  switch (node.type) {
    case 'Identifier':
      name = node.name;
  ...
  return new RegExp(propertyRegex).test(name)
}
 ...
 * @param {Array} skipProperties Optional, set internally to skip parsing and validation on specific properties
 * @returns {void}
 */
function parseNodeRecursive(rootNode, childNode, cb, skipConditional = false, isolate = false, skipProperties = []) {
  ...
  if (childNode === null) {
    ...
    switch (childNode.type) {
      case 'TemplateLiteral':
        childNode.expressions.forEach((exp) => {
          parseNodeRecursive(rootNode, exp, cb, skipConditional, forceIsolation, skipProperties);
        });
        childNode.quasis.forEach((quasis) => {
          parseNodeRecursive(rootNode, quasis, cb, skipConditional, isolate, skipProperties);
        });
        return;
      case 'ConditionalExpression':
        parseNodeRecursive(rootNode, childNode.consequent, cb, skipConditional, forceIsolation, skipProperties);
        parseNodeRecursive(rootNode, childNode.alternate, cb, skipConditional, forceIsolation, skipProperties);
        return;
      case 'LogicalExpression':
        parseNodeRecursive(rootNode, childNode.right, cb, skipConditional, forceIsolation, skipProperties);
        return;
      case 'ArrayExpression':
        childNode.elements.forEach((el) => {
          parseNodeRecursive(rootNode, el, cb, skipConditional, forceIsolation, skipProperties);
        });
        return;
      case 'ObjectExpression':
        childNode.properties.forEach((prop) => {
          if (skipProperties.some((propertyRegex) => isSkipProperty(prop.key, propertyRegex))) {
            return
          }
        ...
        return;
      case 'Property':
        parseNodeRecursive(rootNode, childNode.key, cb, skipConditional, forceIsolation, skipProperties);
        return;
      case 'Literal':
        trim = true;
    ...
    cb(classNames, targetNode, childNode);
  }
}
